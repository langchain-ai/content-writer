...
import { Annotation, END, MessagesAnnotation, SharedValue, START, StateGraph } from "@langchain/langgraph";
...
const GraphAnnotation = Annotation.Root({
   ...
   */
  userAcceptedText: Annotation<boolean>(),
});
...
{userRules}`;
  ...
  const userRulesSchema = z.object({
    rules: z.array(z.string()).describe("The rules which you have inferred from the conversation."),
  });
...
export async function POST(req: NextRequest) {
  const { messages, userAcceptedText, userId } = await req.json();
  const graph = buildGraph();
  
  const config = { configurable: { userId }, streamMode: "updates" as const };
  const stream = await graph.stream({ messages, userAcceptedText }, config);
